...
checkDrainageareaErrors_static <- function(file.output.list,mapping.input.list,
                                    #sub1.plot,
    ...
    if (if_verify_demtarea_maps=="yes"){
      ...
      if (existGeoLines & existlineShape){# map if shape files available
        ...
        title_name <- c("Pre-calculated DEMTAREA","HYDSEQ","new HYDSEQ for unmatched areas","AreaRatio_New:Old")
          ...
          path_checkDrainageareaErrorsChild<-file_path_as_absolute(paste0(path_master,"checkDrainageareaErrorsChild_static.Rmd"))
          
          reportPath<-paste0(path_master,"checkDrainageareaErrors_static.Rmd")
            
          filename<-gsub("pdf","html",filename)


            
            rmarkdown::render(paste0(path_master,"checkDrainageareaErrors_static.Rmd"),
              params = list(
