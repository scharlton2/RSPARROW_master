...
path_mapAttrChild<-params$path_mapAttrChild
  path_classvarChild<-params$path_classvarChild
  path_classLandChild<-params$path_classLandChild
  path_contiguousChild<-params$path_contiguousChild
...
title<-paste(run_id,"_diagnostic_plots.pdf \n ##Document Contents",sep="")
title<-paste("#",title,"\n")
cat(title,strExplanation,sep="\n")
```
...
rmd <- sapply(
  ...
  function(s) {
   knit_expand(path_mapAttrChild, s = s)
  }
)
...
markerText<-"~paste('</br> Observed Load: ',Obs,
                   ...
                   '</br> waterid :',waterid)"
data<-data.frame(predict = predict, Obs = Obs, waterid = sitedata$waterid_for_RSPARROW_mapping)
...
p1 <- p1 %>% add_trace(data = data, x = ~Obs, y = ~Obs, 
  ...
  markerText<-"~paste('</br> Observed Yield: ',yldobs,
                   ...
                   '</br> waterid :',waterid)"
data<-data.frame(yldpredict = yldpredict, yldobs = yldobs, waterid = sitedata$waterid_for_RSPARROW_mapping)
...
#  
  markerText<-"~paste('</br> Log Residual: ',Resids,
                   '</br> Predicted Load: ',predict,
                   '</br> waterid :',waterid)"
data<-data.frame(predict = predict, Resids = Resids, waterid = sitedata$waterid_for_RSPARROW_mapping)
 ...
 p3 <- p3 %>% layout(shapes = list(hline(0)))
  ...
  #abline(h=0,col=2)
     ...
     markerText<-"~paste('</br> Log Residual: ',Resids,
                   '</br> Predicted Yield: ',yldpredict,
                   '</br> waterid :',waterid)"
data<-data.frame(yldpredict = yldpredict, Resids = Resids, waterid = sitedata$waterid_for_RSPARROW_mapping)#
...
p1 <- plotlyLayout(NA,Resids, log = "", nTicks = 4, digits = 0,
  ...
  markerText<-"~paste('</br> Standard Residual: ',standardResids,
                   ...
                   '</br> waterid :',waterid)"
data<-data.frame(standardResids = standardResids, waterid = sitedata$waterid_for_RSPARROW_mapping)
...
p3 <- p3 %>% add_trace(data = data, x = ~x.norm, y = ~y.theo, 
 ...
 # lines(lwx,lwy,col=2)
   markerText<-"~paste('</br> Squared Log Residual: ',Resids2,
                   '</br> Predicted Load: ',predict,
                   '</br> waterid :',waterid)"
  data<-data.frame(Resids2 = Resids2,predict = predict, waterid = sitedata$waterid_for_RSPARROW_mapping)
...
#  lines(ppredict,ppredict,col="red")
   markerText<-"~paste('</br> Squared Log Residual: ',Resids2,
                   '</br> Predicted Load: ',predict,
                   '</br> waterid :',waterid)"
  data<-data.frame(ppredict = ppredict,predict = predict, waterid = sitedata$waterid_for_RSPARROW_mapping)
...
rmd <- sapply(
  1:(length(classvar)),
  function(k) {
   knit_expand(path_classvarChild, k = k)
  }
)
rmd <- paste(rmd, collapse = "\n")
}
...
rmd2 <- sapply(
  1:(length(classvar2)),
  function(l) {
   knit_expand(path_classLandChild, l = l)
  }
)
...
rmd3 <- sapply(
  1:(length(grp)),
  function(i) {
   knit_expand(path_contiguousChild, i = i)
  }
)
...
```{r, eval=TRUE, echo=FALSE,warning=FALSE, message = FALSE, fig.height=8, fig.width=8}  
  ...
  markerText<-"~paste('</br> Observed Load: ',Obs,
                   ...
                   '</br> waterid :',waterid)"
data<-data.frame(ppredict = ppredict, Obs = Obs, waterid = sitedata$waterid_for_RSPARROW_mapping)
...
p1 <- p1 %>% add_trace(data = data, x = ~Obs, y = ~Obs, 
  ...
  markerText<-"~paste('</br> Observed Yield: ',pyldobs,
                   ...
                   '</br> waterid :',waterid)"
data<-data.frame(pyldpredict = pyldpredict, pyldobs = pyldobs, waterid = sitedata$waterid_for_RSPARROW_mapping)
...
#  # mass residual plot
  markerText<-"~paste('</br> Log Residual: ',pResids,
                   '</br> Predicted Load: ',ppredict,
                   '</br> waterid :',waterid)"
data<-data.frame(ppredict = ppredict, pResids = pResids, waterid = sitedata$waterid_for_RSPARROW_mapping)
 ...
 p3 <- p3 %>% layout(shapes = list(hline(0)))
  ...
  # yield residual plot
     markerText<-"~paste('</br> Log Residual: ',pResids,
                   '</br> Predicted Yield: ',pyldpredict,
                   '</br> waterid :',waterid)"
data<-data.frame(pyldpredict = pyldpredict, pResids = pResids, waterid = sitedata$waterid_for_RSPARROW_mapping)#
...
#  qqline(pResids, col = 2)
  ...
  markerText<-"~paste('</br> Log Residual: ',pResids,
                      '</br> Theoretical Value: ',y.theo,
                   '</br> waterid :',waterid)"
data<-data.frame(pResids = pResids, waterid = sitedata$waterid_for_RSPARROW_mapping)
...
p3 <- p3 %>% add_trace(data = data, x = ~x.norm, y = ~y.theo, 
 ...
 # lines(lwx,lwy,col=2)
   markerText<-"~paste('</br> Squared Log Residual: ',Resids2,
                   '</br> Predicted Load: ',ppredict,
                   '</br> waterid :',waterid)"
  data<-data.frame(Resids2 = Resids2,ppredict = ppredict, waterid = sitedata$waterid_for_RSPARROW_mapping)
...
```{r, eval=TRUE, echo=FALSE,warning=FALSE, message = FALSE, fig.height=8, fig.width=8}  
  ...
  #for (i in 1:length(grp)) {
  #  plotmrb(grp[i],class[,1],Obs,ppredict,pyldobs,pyldpredict,pResids, sitedata)
  #}
