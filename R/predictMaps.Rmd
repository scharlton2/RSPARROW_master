...
save(s,file="D:/s")
    save(plotdata,file="D:/plotdata137")
            if ((is.na(map_years) & is.na(map_seasons)) | (!is.na(map_years) & map_years=="mean") | (!is.na(map_seasons) & map_seasons=="mean")){
...
if (!is.na(map_years) & map_years=="mean" & !is.na(map_seasons) & map_seasons=="mean"){
  subTitle<-titleStr
}else if (is.na(map_seasons) | map_seasons=="mean"){
  subTitle<-y
}else if (is.na(map_years) | map_years=="mean"){
  subTitle<-s
}else{
  subTitle<-paste(y,s)
}
...
}else if (is.na(mapPageGroupBy) & !is.na(map_seasons) & map_seasons=="mean"){
  plotPageData<-dmapfinal[dmapfinal$year %in% plotSub$year,]
}else if (is.na(mapPageGroupBy) & !is.na(map_years) & map_years=="mean"){
