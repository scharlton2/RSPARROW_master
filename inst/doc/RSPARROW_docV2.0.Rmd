...
author: "Lillian Gorman Sanisaca and Richard B. Alexander, U.S. Geological Survey"
date: "`r format(Sys.time(), '%d %B, %Y')`(Version 2.0)"
output: 
  pdf_document: 
...
\textcolor{red}{This documentation describes the steps necessary to estimate the original static version of the model (long-term mean annual loads; e.g., Smith et al., 1997) as well as the recently developed dynamic (time-dimensioned) version for modeling mean annual or mean seasonal loads. Both models provide reach-level predictions (and uncertainties) of the mean water-quality loads, yields, and flow-weighted concentrations. The predictions also include the shares of the load attributable to individual upstream sources and predictions of the mass quantities of the total load and individual sources that are delivered to downstream water bodies.}

\textcolor{red}{The dynamic model, available for the first time in the 2.0 version of RSPARROW, supports applications with time-dimensioned input data for modeling seasonal and annual variations in sources and transport properties in watersheds. The input data of time-varying conditions can be expressed as mean seasonal (four mean seasonal values at each monitoring site, computed as seasonal averages of the observations across multiple years), mean annual (multiple continuous or discrete mean annual values, computed as mean averages of the observations across multiple seasons), or a time series of seasonal observations across multiple years and seasons. The dynamic model in the 2.0 version is a simplified model specification that does not account for the storage and release (i.e., temporal lags) of physical or chemical substances in watersheds over time. The calibrated model consists of a single set of coefficients estimated for the collection of observations from all seasons and/or years. Contaminant storage processes, which are available in the current SAS SPARROW model code, will be added to a future version of RSPARROW. }
...
**\textcolor{red}{The 2.0 RSPARROW version provides the following new enhancements to the earlier 1.1 version: }**

* \textcolor{red}{Executes dynamic SPARROW models for annually and/or seasonally varying input data. The input data can be expressed as mean seasonal (four mean seasonal values at each monitoring site, computed as seasonal averages of the observations across multiple years), mean annual (multiple continuous or discrete mean annual values, computed as mean averages of the observations across multiple seasons), or a time series of seasonal observations across multiple years and seasons. The dynamic version is capable of simulating temporal changes in both contaminant sources and land-to-water delivery factors (e.g., climate).  Note that the model doesn’t include internal process components (or separate model coefficients) to account for the temporary storage and release of water and water-quality constituents, processes that can cause temporal lags in transport across time periods in watersheds (e.g., Schmadel et al., 2021). }

* \textcolor{red}{Provides graphical displays of temporally varying output to support the dynamic models, including annual and seasonal diagnostic maps and plots as well as interactive multi-panel stream and catchment water-quality prediction map displays for separate seasons and years. }

* \textcolor{red}{Supports the execution of additional types of management scenarios for static and dynamic models in the RShiny Decision Support System (DSS). This includes scenarios that evaluate the stream water-quality effects of:  (1) percentage changes in untransformed land-to-water delivery variables (e.g., temperature, precipitation); and (2) user-supplied forecasted changes in source and/or land-water delivery variables that are input from an external CSV file. The capabililty to input a user's CSV file allows, for example, the evaluation of the stream water-quality effects of forecasted changes in land use or climate conditions (e.g., temperature, precipitation) as reported by global climate models. User control over forecasted changes input from a CSV file also allows the evaluation of scenarios that entail changes in land-water delivery variables that are transformed in the calibrated model (e.g., logarithmic). In these cases, a relative or absolute change can be externally applied to the untransformed delivery variable by the user, with the variable subsequently transformed and stored in the external CSV file prior to input to the RSPARROW DSS. }
...
\textcolor{red}{Three directories contain the contents of RSPARROW (See Figure \ref{directory1} } for the contents of the RSPARROW directories as installed on a user's computer):
...
\textcolor{red}{RSPARROW 2.0 was tested for compatibility with versions of R, RStudio, and the required R library functions available at the time of public release (September 2023): These include R-4.2.2, RStudio 2022.12.0 (Build 353),} and the required versions of R library dependencies that are listed in the DESCRIPTION file located in the "RSPARROW_master" directory. RSPARROW may operate with more current versions of the software, but full compatibility of RSPARROW with newer versions is not guaranteed.


\textcolor{red}{SELECTED PORTIONS OF THE FOLLOWING TEXT HAS BEEN REVISED}

**The required software can be obtained as follows:**

* *R:* The current and past versions are available from the Cran server at *https://cran.r-project.org/bin/windows/base/old/*. We recommend installation of R-4.2.2, using the zipped version included with the RSPARROW 2.0 download from the USGS GitLab repository. The included R version contains all R libraries and dependencies that are required to execute RSPARROW 2.0. The R directory should be unzipped to a location on your PC that does not require Administrator rights. RStudio access to the R directory is achieved by setting the R directory path in the *"Tools >> Global Options >> R version:"* (see section 1.3.4 below for details). 
...
* *R libraries and dependencies:* Advanced R users can install R for versions >=4.2.2 from CRAN and automatically install R dependencies for RSPARROW when executing the RSPARROW control script (located in the *"UserTutorial/results"* directory) in RStudio. Execution of the control script automatically installs the most recent versions of the required R library packages and functions in the user's default R library as specified in RStudio (see the next subsections for instructions on setting RStudio defaults and information on the installation of RSPARROW library dependencies). If the included version of R-4.2.2 (with pre-installed RSPARROW R library dependencies) is used, then the auto-installation steps are not executed.
...
More advanced users, who choose to manually download R from CRAN and install the RSPARROW R library dependencies, should be attentive to the following messages and prompts associated with the R package installations when opening and executing the RSPARROW control script in RStudio:
...
* *R package installation and updating:*  The required RSPARROW library dependencies (and the most recent library versions) are automatically installed when the control script is initially executed with a newly installed version of R that excludes the RSPARROW R library dependencies (>=R-4.2.2). In subsequent executions of the control script, no updates are made to the library dependencies (see Chapter subsection 4.4.12 for details).

* *R package installation from source:*  If the following question appears in a pop-up box in RStudio during the installation of R library dependencies, users should select *"No"* as the response: *"Do you want to install from sources the packages which need compilation?"*. A *"No"* response installs the most current R packages from binary versions. The selection of *"Yes"* requires that users have installed *Rtools42* (an R developer package) on the Windows operating system; this enables the compilation and installation of R packages from their source.

* *Resolution of library installation problems:* Users have periodically encountered R library dependencies that will not install properly in RStudio. The problem is often resolved by:  (1) deleting the current R version and installing a new R base version (i.e., >=4.2.2), and (2) executing the RSPARROW control file, which will auto-install the required library dependencies to the new R version. Alternatively, users can install the version of R-4.2.2 with RSPARROW R library dependencies that is included with the RSPARROW download.
...
\textcolor{red}{THE FOLLOWING 2 PARAGRAPH HAVE EDITED TEXT}
...
Stream and catchment maps of model predictions, or other variables listed in the *dataDictionary.csv*, may be created by users with the `master_map_list` setting. The maps are stored as PDF files (with non-interactive features) or HTML files (with interactive features) in the *"maps"* subdirectory. Alternatively, the map setting `enable_shinyMaps<-"yes"` (section 9 of the control script) can be used to enable the RShiny mapper, allowing users to interactively select and view predictions and optionally save maps of prediction metrics or other variables in the *"maps/Interactive"* subdirectory.  
...
\textcolor{red}{SELECTED PORTIONS OF TABLE 2 CAPTION AND CONTENTS HAVE BEEN REVISED}
...
\textcolor{white}{.}
 (run_id)/scenarios/(scenario_name)           (scenario_name)_(run_id)_predicts_load_scenario.csv               
                                              ...
                                              **batch**_**(time).RData**           
.
 (run_id)/scenarios/(scenario_name)           (scenario_name)\_(run_id)\_(variable_name).pdf (.html)                                                
  ...(/Stream)...(/Catchment)                          
\textcolor{white}{.}
...
\textcolor{white}{.}                          
 (run_id)/estimate                            meanload.html                                             
...(/diagnostic_PlotsNLLS_dynamic             meanconc.html
/mapSiteAttributes)
\textcolor{white}{.}                          
 (run_id)/estimate                            ModelEstimationLogResiduals_all.html                                             
...(/diagnostic_PlotsNLLS_dynamic             ModelEstimationLogResiduals_threshold-above.html
/residualMaps)                                ModelEstimationLogResiduals_threshold-below.html
                                              ModelEstimationObs.PredRatio_all.html
                                              ModelEstimationObs.PredRatio_threshold-above.html
                                              ModelEstimationObs.PredRatio_threshold-below.html
                                              ModelEstimationStandardizedResiduals_all.html
                                              ModelEstimationStandardizedResiduals_threshold-above.html
                                              ModelEstimationStandardizedResiduals_threshold-below.html
                                              ModelSimulationLogResiduals_all.html
                                              ModelSimulationLogResiduals_threshold-above.html
                                              ModelSimulationLogResiduals_threshold-below.html
                                              ModelSimulationObs.PredRatio_all.html
                                              ModelSimulationObs.PredRatio_threshold-above.html
                                              ModelSimulationObs.PredRatio_threshold-below.html
\textcolor{white}{.}
...
...(/validation_plots_dynamic)                ModSimulation_Box_and_Quantile_Resid.html
                                              ...
                                              ModSimulation_Obs_v_Pred.html
\textcolor{white}{.}                          
 (run_id)/estimate                            ModelSimulationLogResiduals_all.html                                            
...(/validation_plots_dynamic                 ModelSimulationLogResiduals_threshold-above.html 
/residualMaps)                                ModelSimulationLogResiduals_threshold-below.html
                                              ModelSimulationObs.PredRatio_all.html
                                              ModelSimulationObs.PredRatio_threshold-above.html
                                              ModelSimulationObs.PredRatio_threshold-below.html
--------------------------------------------  ----------------------------------------------------------                                                       
...
The *dataDictionary.csv* ensures global system-wide access to the *sparrowNames* variables (Table 4) during an RStudio session. By executing the control script within an RStudio session, variables listed in the *dataDictionary.csv*, including those modified in *userModifyData* script, are stored internally and are availabe for RSPARROW operations, including estimation, prediction, mapping, and source-change scenario evaluations (see subsection 3.5).
...
\textcolor{red}{THE FOLLOWING TABLE HAS NEW TEXT FOR THESE VARIABLES: } waterid, mapping_waterid, season, year
...
\textcolor{red}{1. Execute at least one of the tutorial models (Chapter 6 or 7) to ensure that the required RSPARROW software (R, RStudio, R library dependencies) has been properly installed. See Chapter subsection 1.3 for details on the software installation. }
...
\textcolor{red}{THE FOLLOWING PARAGRAPH IS NEW TEXT}
...
Users can also evaluate regional variations in model performance and coefficient sensitivites by enabling diagnostic control settings for user-specified regional classification variables that define physiographic regions, political jurisdictions, or land use types (see the discussion in subsection 4.4.5 and the `classvar` and `class_landuse` settings in section 5 of the control script). By default, variations in sensitivities and in the ratio of the observed values to the model predictions are plotted in the *(run\_id)\_diagnostic_sensitivity.html* and *(run\_id)\_diagnostic_plots.html* files, respectively, for different watershed sizes, based on total drainage area decilies.
...
\textcolor{red}{THE FOLLOWING SECTION HAS NEW TEXT}
...
```{r, eval=FALSE}
  ...
  diagnosticPlots_timestep<-"season"  # Options:  NA, “season”, or “year”
                                      #  Controls the timestep of observations output to
                                      #  files in 'diagnosticPlotsNLLS_dynamic' directory.
  diagnostic_timeSeriesPlots<-"no"    # Options:  “yes”, “no”
...
\textcolor{red}{THE FOLLOWING 2 PARAGRAPHS ARE NEW TEXT}
...
```{r, eval=FALSE}
  ...
  add_plotlyVars<-c("waterid","rchname","staid") # "year" and/or "season" 
                                                 #   added by default for
                                                 #   dynamic model
  showPlotGrid<-"no"
```
...
\textcolor{red}{THE FOLLOWING SECTIONS HAVE NEW TEXT AND CONTROL SETTINGS}
...
#---------------------------------------------------
  ...
  #  set years/seasons of data to include or select "all" to map all years/seasons
  #  set "mean" for map_years/map_seasons to map an average of all years/seasons 
  #       for each season/year selected
  #  set NA for static model

# Options:  NA; year1; c(year1, year2, …);  “all”; “mean”; “median”; “min”; “max”
  map_years <-c(2001,2002)      

# Options:  NA; season1; c(season1, season2, …); “all”; “mean”; “median”; “min”; “max”
  map_seasons <- “all”   
...
Table 7 provides a detailed illustration of the types of plots that are output by different combinations of these four control settings. The settings control the format of HTML files output to the three subdirectories listed above.
...
\textcolor{red}{THE FOLLOWING SECTION HAS REVISED AND NEW TEXT}
...
\textcolor{red}{END OF NEW/REVISED TEXT}   
...
\textcolor{red}{THE FOLLOWING SECTION 4.4.9.1.2 IS NEW}
...
\textcolor{red}{END OF NEW SECTION}   
...
To enable the RShiny mapper for a previously executed model within an existing or new RStudio session, users should run the following statements in the RStudio Console window. The `runBatchShiny` control setting will enable the mapper for the results of a prior model specified according to the full path name that is listed in the function argument, as illustrated below. 
...
A user can also enable RShiny in a specified browser for a previously executed model by specifying the browser path name (`path_ShinyBrowser`) for the second argument of the `runBatchShiny` control setting, as illustrated below. 
...
\textcolor{red}{THE FOLLOWING SECTION 4.4.9.2.1 IS NEW TEXT}
...
# Select CSV file to input forecasted values for scenario
 forecast_filename <- NA    # no file selected
			                      # “forecastScenario.csv” (enter name to select file 
                            # option)
			    
# Specify if sparrowNames (TRUE) or data1UserNames (FALSE) are used in the forecast 
#                          CSV file
...
\textcolor{red}{END OF NEW subsection}
...
#--------------------------------------------------------------------------------
  ...
  # List the source and/or delivery variable variables evaluated in the change scenarios.
  scenario_sources <- NA
  ...
  # Source/delivery variable adjustment factors (increase or decrease) applied 
  #  to "all reaches" in the user-specified watersheds. Enter a factor of 0.1 or 1.1 
  #  to obtain a 10% reduction or increase in a source, respectively. Order must
  #  be consistent with the 'scenario_sources'.
  scenario_factors <- NA
  scenario_factors <- c(0.20,1.1,0.25)
...
```
\textcolor{red}{THE FOLLOWING PARAGRAPH HAS REVISED TEXT}
...
\textcolor{red}{END OF REVISED TEXT}
...
\textcolor{red}{NEW TEXT FOR DYNAMIC MODEL OUTPUT}
...
\textcolor{red}{ADDED NEW SECTION 5.2.4.9}
...
**sPBias** = Percent Bias for each season, computed as the ratio of the sum of the model residuals (observed load - predicted load) to the sum of the observed load across all calibration sites, and multipled by 100 (see Moriasi et al., 2007). Positive values indicate prediction underestimation bias, whereas negative values indicate prediction overestimation bias.
...
**spPBisas** = Percent Bias for each season, computed as the ratio of the sum of the model residuals (observed load - unconditioned predicted load) to the sum of the observed load across all calibration sites, and multipled by 100 (see Moriasi et al., 2007).
...
There are four major subsections of the output file, including an initial section with user-specified site attribute maps and three sections of model performance diagnostics (plots, maps) for the calibration sites. The model performance diagnostics are reported separately based on the use of conditioned and unconditioned model predictions (see Chapter subsection 4.4.7.1 for full explanation). These provide two informative perspectives on model performance for *estimation* and *simulation*.
...
Using the unconditioned predictions, the plots described in the above subsection (5.2.4.9.2) are replicated (except for the plot of conditioned vs. unconditioned loads) and output to the file.  
...
\textcolor{red}{ADDED NEW subsection}


##### Additional optional diagnostic output for dynamic models

&nbsp;

For dynamic models, users have the option to produce seasonal and/or annual subsets of the various plots and maps that are output to the *(run\_id)\_diagnostic_plots.html* file as described in the subsections above (i.e., the site attribute maps and the performance diagnostic plots and maps). These subsets of the plots and maps are output to separate HTML files, with self explanatory labels in the file name, in the subdirectory *".../estimate/diagnosticPlotsNLLS_dynamic"*; site attribute maps are directed to the subdirectory *"/mapSiteAttributes"* and residual maps are directed to the subdirectory *"/residualMaps"*. 

Note that, in contrast to the other aggregate diagnostic plots and maps (all observations), residual maps are NOT output to the *(run\_id)\_diagnostic_plots.html* file, and are only directed to the subdirectory *"/residualMaps"*.). 

Five control script settings (section 8 of the control script) govern the format of the output of annual and/or seasonal plots and maps:  `map_years`, `map_seasons`, `mapsPerPage`, `mapPageGroupBy`, and `diagnosticPlots_timestep`. See Chapter subsection 4.4.8.4 for a detailed description of the use of the control settings to create plots and maps by season and/or year. 

Additionally, users have the option to output time series plots for the calibration stations. These are governed by two control script settings (section 8 of the control script):  `diagnosticPlots_timestep` (options:  NA, “season”, or “year”) and `diagnostic_timeSeriesPlots` (Options: “yes”, “no”). The plots are output to the file *Obs_v_Pred_TimeSeriesPlots.html*, located in the *".../estimate/diagnosticPlotsNLLS_dynamic"* subdirectory. 
...
For dynamic models, users have the option to produce seasonal and/or annual subsets of the various plots described above. The plots are output to three separate HTML files, with self explanatory labels in the file name, in the subdirectory *".../estimate/diagnostic_sensitivity_dynamic"*. 
...
For dynamic models, users have the option to produce seasonal and/or annual subsets of the various plots and maps that are output to the *(run\_id)\_validation_plots.html* file as described above (i.e., the performance diagnostic plots and maps). These subsets of the plots and maps are output to separate HTML files, with self explanatory labels in the file name, in the subdirectory *".../estimate/validation_plots_dynamic"*; residual maps are directed to the subdirectory *"/residualMaps"*. 

Note that, in contrast to the other aggregate diagnostic plots and maps (all observations), residual maps are NOT output to the *(run\_id)\_validation_plots.html* file, and are only directed to the subdirectory *"/residualMaps"*.). 
...
For dynamic models, users have the option to produce seasonal and/or annual subsets of the various plots described above. The plots are output to four separate HTML files, with self explanatory labels in the file name, in the subdirectory *".../estimate/diagnostic_spatialautocor_dynamic"*. 
...
These HTML and PDF files are also output from the RShiny interactive DSS mapper (i.e., `enable_ShinyApp<-"yes"`) when the maps are saved using the *"Save Map"* button (see section 9 of the control script and Chapter subsection 4.4.9 for details). The files are written to subdirectories ("Stream" and/or "Catchment") within the scenario directory that is named by the user (i.e., entry for the *"Enter Scenario Name"* option).  
...
All of the control files and model output files are available for the eight models demonstrated in the tutorial. These model files are stored in the *"run_id"* subdirectories under the "UserTutorial/results" directory. The highlighted results (text, plots, maps) in this chapter can be viewed in the *(run\_id)\_diagnostic_plots.html* and *(run\_id)\_summary.txt* files in each of the "run_id" (model1 to model8) subdirectories.
...
Note that the summary statistics for model performance and coefficient estimates as well as the plots and maps shown in this section for model 1 can be obtained from the *“model1_summary.txt”* and *"Model1_diagnostic_plots.html"* files in the *“/results/Model1/estimate”* subdirectory.
...
\textcolor{red}{THE FOLLOWING PARAGRAPHS HAVE SOME EDITED AND NEW TEXT--added info on options of land-water delivery variables and use of external CSV file. }
...
Users can also create additional time series plots of the predictions, residuals, and other monitoring station attributes by using R or other ploting software to access the data output to the *(run\_id)\_residuals.csv* file in the *“UserTutorial_Dynamic/results/(run_id)/estimate”* directory. 
...
Depending on the timestep associated with the dynamic model, a menu setting, with dropdown check-box entries, appears with the label *“year(s)”* and/or *“season(s)”* (see examples of the dropdown entry options for these settings in Figures \ref{dyntutorial_4} and \ref{dyntutorial_5}). A maximum of four maps is allowed for display on the DSS page for any single execution (i.e., “Generate Map” button) of user settings from the menu. Therefore, users can select a maximum of four years or seasons for display for a multi-year seasonal model, such as the Tampa Bay TP model. 
...
See Table 7 in Chapter subsection 4.4.8.4 for additional guidance and examples of the map display options based on the settings available in the sparrow control script. There is no limit placed on the number of maps that can be output using the control script settings.  
...
This **“change-percentage scenario”** illustrates the effects on TP stream loads of a 10 percent reduction in seasonal fertilizer use (`FertP_Seas`). The DSS menu settings and the map display of the reach-level effects of fertilizer reduction on TP load are shown in Figure \ref{dyntutorial_7}. The base year and season for application of the scenario is Summer 2014, the most recent season and year available in the record used to calibrate the model. The check-box *“Dynamic Mapping Display Settings”* are *“2014”* for *“year(s)”* and *“summer”* for *“season(s)”*, which will generate a map in the DSS display that is consistent with the simulation timestep. 

To specify the timestep (year and/or season) for application of the scenario simulation (as illustrated in Figure \ref{dyntutorial_7}), the following menu entries are required: 

* Select the option *“selected reaches”* for the menu setting labeled *“Select reaches for applying the scenario (within targeted watersheds)”*. Note that selection of the option *“all reaches”* for this setting does not allow the user to enter values for the timestep. 

* Select the option *“yes”* or *“no”* for the menu setting labeled *“Apply same reach selection criteria to all selected changed variables (yes/no)”*. 

  * A *“yes”* entry, as illustrated in Figure \ref{dyntutorial_7}, displays two tables in the menu. The first table allows users to select the changed model variable (`FertP_Seas`) and enter the percentage quantity for application of the percentage change (see table label *“Select Model Explanatory Variables and Percent Change Factors”*). The second table allows users to specify additional criteria for the scenario, including the timestep entries (see table label *“Reach Selection Criteria”*), which apply to all model explanatory variables (i.e., sources and/or delivery variables). As shown in Figure \ref{dyntutorial_7}, the year and season were entered into the second table, using the *“AND”* separator to ensure that both conditions are applied to all of the reaches. 
...
The results of the scenario simulation as shown in Figure \ref{dyntutorial_7} display the relative change in TP load in streams.  These show that a 10 percent reduction in seasonal fertilizer would be expected to result in reductions in stream TP load of less than about 8 percent, with most changes less than 2 percent. Predicted reductions of substantially less than 10 percent in many of the Tampa Bay streams is an indication that other sources that are unchanged in the scenario contribute considerably to stream phosphorus loads. 


\begin{figure}
\includegraphics[width=2.0\linewidth]{figures/tutorial_dynamic/figures_dyntutorial_7.jpg}
\caption{\textbf{RShiny DSS interactive mapper interface for the Tampa Bay seasonal total phosphorus model, with settings to evaluate a percentage-change scenario for a 10 percent reduction in the seasonal fertilizer source variable. } }
\label{dyntutorial_7}
\end{figure}
...
Four scenarios are presented in this subsection that illustrate the effects of a changes in climate and land-cover conditions on stream TP loads. The model explanatory variables evaluated in these scenarios include the seasonal mean precipitation (`logPPT`), mean temperature (`logAirTemp`), and fertilizer use (`FertP_Seas`).  The DSS menu settings and the map display of the reach-level effects of changes in the explanatory variables on TP load are shown in Figures \ref{dyntutorial_8} to \ref{dyntutorial_11}. A base year of 2014 is used in the scenarios to facilitate comparisons with the predictions for future years; 2014 is the most recent year available in the record that was used to calibrate the model. The methods and references used to derive the forecasts of future climate and land-cover conditions associated with the explanatory model variables are described in the next subsection (7.2.2.2.4).

The temporal menu selections for the *“Dynamic Mapping Display Settings”*, which control the map display of the results, include *“2014”* for *“year(s)”* for all of the scenarios; the *“season(s)”* setting is *“spring”* for the first scenario (Figure \ref{dyntutorial_8}), whereas all four seasons are selected for display for the subsequent scenarios (Figures \ref{dyntutorial_9} to \ref{dyntutorial_11}). For each of the scenarios, the *“year”* and *“season”* settings generate prediction maps in the DSS display that are consistent with the simulation timestep that is implicitly reflected in the values of the *waterid* variable (unique reach and timestep variable) in the external CSV file.

To input the external CSV forecasting files to the DSS, the following menu entries are required, as shown in Figures \ref{dyntutorial_8} to \ref{dyntutorial_11}:
...
\begin{figure}
\includegraphics[width=2.0\linewidth]{figures/tutorial_dynamic/figures_dyntutorial_8.jpg}
...
\begin{figure}
\includegraphics[width=2.0\linewidth]{figures/tutorial_dynamic/figures_dyntutorial_9.jpg}
...
*Long-term change in land-cover conditions (Figure \ref{dyntutorial_10})*
...
\begin{figure}
\includegraphics[width=1.8\linewidth]{figures/tutorial_dynamic/figures_dyntutorial_10.jpg}
...
The RSPARROW library dependencies include the standard R packages listed in the DESCRIPTION file located in the *"RSPARROW_master"* directory. The 2.0 version of RSPARROW with an R-4.2.2 installation was tested using the versions of the library dependencies listed in the DESCRIPTION file. 

We recommend installation of R-4.2.2, using the zipped version included with the RSPARROW 2.0 download from the USGS GitLab repository. The included R version contains all R libraries and dependencies that are required to execute RSPARROW 2.0. 

Advanced R users can alternatively install R for versions >=4.2.2 from CRAN and automatically install R dependencies for RSPARROW when executing the RSPARROW control script. Execution of the control script runs the R statement shown below (`devtools::install_deps`), which automatically installs (on the first run of the script) the most recent versions of the required R functions in the user's default R library (as specified in RStudio). The installed R library packages are then loaded into the RStudio session by the R statement `devtools::load_all`. 
...
In subsequent executions of the control script, no updates are made to the library dependencies (see Chapter subsection 4.4.12 for details). Users are not required to update the R packages to versions that are more recent than those listed for the RSPARROW compatibility versions in the DESCRIPTION file. However, when a more recent version of R is installed (>R-4.2.2), the latest versions of the library dependencies will be automatically installed with the initial execution of the control script. Note that RSPARROW compatibility with updated R package versions (more recent than those listed in the DESCRIPTION file) is not guaranteed.

Note that RStudio will print the following message in a yellow banner when the RSPARROW control script is opened in RStudio for the first time with a new R version:  *"Packages devtools and rstudioapi required but are not installed."* Users should ignore this message and instead execute (*"Source"*) the control script, which will install these and other required R packages (**User caution:** Package installation using the yellow banner prompt may exclude certain required packages such as *"backports"*). 

Users are also directed to select *"No"* as the response to the following question, if it appears in a pop-up box in RStudio during the installation of R library dependencies: *"Do you want to install from sources the packages which need compilation?"*. A *"No"* response installs R packages from binary versions, which may not include the most recent version of a package. The selection of *"Yes"* requires that users have previously set up *Rtools42* (an R developer package) to compile R packages from their source, otherwise the installation of R packages from their source will fail. 
...
**sparrowSetup** -  Used to create, import, check, and format control files for SPARROW execution (42).  

**sparrowExec** - Core functional routines that execute SPARROW models (55).
...
**errorTrap** - Tests control settings and files for critical errors and stops execution if a critical error is detected (24).

**utility** - Used throughout the RSPARROW system to perform minor computational or administrative tasks; these include variable input arguments (31).

**lists** - Creates control setting lists that organize the control settings for import into multiple functions and routines (8).

**rmd** - R Markdown functions for creating HTML output of plots and maps (27).
...
**external** - Functions that can be used outside of the RSPARROW system and/or within the *userModifyData.R* control script (7).

**dynamic** - R routines and Markdown (rmd) functions for executing dynamic versions of the SPARROW model (19). 
...
\caption{\textbf{RSPARROW library functions and routine types, organized by 12 functional categories. Users can access this table in the RSPARROW funcTypes.csv file.} }
\label{functionType1}
\end{figure}

\begin{figure}
\includegraphics[width=1.3\linewidth]{figures/FigFunctionTypes2.jpg}
\caption{\textbf{RSPARROW library functions and routine types, organized by 12 functional categories (continued). Users can access this table in the RSPARROW funcTypes.csv file.} }
\label{functionType2}
\end{figure}
...
\textcolor{red}{SELECTED PORTIONS OF THE FOLLOWING LIST OBJECT HAS BEEN REVISED}
...
\textcolor{red}{THE FOLLOWING TWO BULLETS ARE NEW}

* *Estimation of contaminant storage in watersheds in dynamic SPARROW models.* The dynamic model specification in RSPARROW 2.0 is a simplified version of the current SAS dynamic SPARROW model, and does not account for the storage and release (i.e., temporal lags) of physical or chemical substances in watersheds over time. These contaminant storage processes will be added to a future version of RSPARROW by implementing the storage process methods in the current SAS SPARROW model code.

* *Adjustment of the model coefficient standard errors of dynamic models for serial correlation.* Serial correlation can exist in the time series of the residuals of dynamic SPARROW models, which can lower the statistical significance of the model coefficients as compared to that for conventional SPARROW models that assume temporal independence in the residuals (e.g., Ator et al., 2019; Schmadel et al., 2021). Ancillary SAS SPARROW code is currently available to adjust the standard errors of the coefficients of the dynamic models for serial correlation (G. Schwarz, USGS, written comm., 2022). A future version of RSPARROW will incorporate these methods for use with RSPARROW dynamic models. 
...
White, C., M. Barik, J. Srikishen, M. Al-Hamdan, and W. Crosson (2022), “RSPARROW Atmospheric Post Processing Tool”, Universities Space Research Association. *ADD URL HERE*
