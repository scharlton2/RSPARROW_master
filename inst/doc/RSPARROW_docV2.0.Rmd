---
title: "RSPARROW 2.0:  An R System for SPARROW Modeling"
...
\textcolor{red}{NEW TEXT ADDED}

**The 2.0 RSPARROW version provides the following new enhancements to the earlier 1.1 version ([Alexander and Gorman Sanisaca, 2019](https://doi.org/10.5066/P9UAZ6FO)): **

* Executes dynamic SPARROW models for annually and/or seasonally varying input data. The input data can be expressed as mean seasonal (four mean seasonal values at each monitoring site, computed as seasonal averages of the observations across multiple years), mean annual (multiple continuous or discrete mean annual values, computed as mean averages of the observations across multiple seasons), or a time series of seasonal observations across multiple years and seasons. The dynamic version is capable of simulating temporal changes in both contaminant sources and land-to-water delivery factors (e.g., climate).  Note that the model doesn’t include internal process components (or separate model coefficients) to account for the temporary storage and release of water and water-quality constituents, processes that can cause temporal lags in transport across time periods in watersheds (e.g., Schmadel et al., 2021). 
...
\textcolor{red}{RSPARROW 2.0 was tested for compatibility with versions of R, RStudio, and the required R library functions available at the time of public release (September 2023): These include R-4.3.1, RStudio 2022.12.0 (Build 353),} and the required versions of R library dependencies that are listed in the DESCRIPTION file located in the "RSPARROW_master" directory. RSPARROW may operate with more current versions of the software, but full compatibility of RSPARROW with newer versions is not guaranteed.
...
* *R:* The current and past versions are available from the Cran server at *https://cran.r-project.org/bin/windows/base/old/*. We recommend installation of R-4.3.1, using the zipped version included with the RSPARROW 2.0 download from the USGS GitLab repository. The included R version contains all R libraries and dependencies that are required to execute RSPARROW 2.0. The R directory should be unzipped to a location on your PC that does not require Administrator rights. RStudio access to the R directory is achieved by setting the R directory path in the *"Tools >> Global Options >> R version:"* (see section 1.3.4 below for details). 
...
* *R libraries and dependencies:* Advanced R users can install R for versions >=4.3.1 from CRAN and automatically install R dependencies for RSPARROW when executing the RSPARROW control script (located in the *"UserTutorial/results"* directory) in RStudio. Execution of the control script automatically installs the most recent versions of the required R library packages and functions in the user's default R library as specified in RStudio (see the next subsections for instructions on setting RStudio defaults and information on the installation of RSPARROW library dependencies). If the included version of R-4.3.1 (with pre-installed RSPARROW R library dependencies) is used, then the auto-installation steps are not executed.
...
* For the *"R version"*, select the *"Change..."* button and choose an already installed 64-bit R version or, as we recommend, browse to the *"R.exe"* executable binary object for the R-4.3.1 version (*".../R-4.3.1/bin/x64/R.exe"*) and select the binary object.   
...
* *Resolution of library installation problems:* Users have periodically encountered R library dependencies that will not install properly in RStudio. The problem is often resolved by:  (1) deleting the current R version and installing a new R base version (i.e., >=4.3.1), and (2) executing the RSPARROW control file, which will auto-install the required library dependencies to the new R version. Alternatively, users can install the version of R-4.3.1 with RSPARROW R library dependencies that is included with the RSPARROW download.
...
\textcolor{red}{NEW TEXT}

Note that the standard errors of the coefficients of dynamic RSPARROW models are not adjusted for serial correlation, which can exist in the time series of dynamic model residuals. Serial correlation in model residuals can cause somewhat underestimated coefficient standard errors, which will result in overestimates of the significance levels (i.e., lower p-values) of SPARROW model coefficients (e.g., Ator et al., 2019; Schmadel et al., 2021). 
...
The tutorial uses a preliminary test-version of a SPARROW dynamic model, developed to simulate total phosphorus (TP) loads in streams of the Tampa Bay watershed, a 5,700 square kilometer basin on the central western coast of the state of Florida that drains to the Gulf of Mexico. The model uses non-propriety data from the USGS (R. Smith, written comm., 2022) and NASA (J. Srikishen, written comm., 2022) that are currently being evaluated with various versions of a SPARROW dynamic TP model for the Tampa Bay watershed. One version of the TP model is presented here; however, the model and data are used for illustration purposes only to demonstrate the features and capabilities of RSPARROW. The model and supporting data should not be used for actual water resource evaluations of TP loads in the Tampa Bay watershed or to support related water management decisions for the watershed or estuary.  
...
The RSPARROW library dependencies include the standard R packages listed in the DESCRIPTION file located in the *"RSPARROW_master"* directory. The 2.0 version of RSPARROW with an R-4.3.1 installation was tested using the versions of the library dependencies listed in the DESCRIPTION file. 

We recommend installation of R-4.3.1, using the zipped version included with the RSPARROW 2.0 download from the USGS GitLab repository. The included R version contains all R libraries and dependencies that are required to execute RSPARROW 2.0. 

Advanced R users can alternatively install R for versions >=4.3.1 from CRAN and automatically install R dependencies for RSPARROW when executing the RSPARROW control script. Execution of the control script runs the R statement shown below (`devtools::install_deps`), which automatically installs (on the first run of the script) the most recent versions of the required R functions in the user's default R library (as specified in RStudio). The installed R library packages are then loaded into the RStudio session by the R statement `devtools::load_all`. 
...
In subsequent executions of the control script, no updates are made to the library dependencies (see Chapter subsection 4.4.12 for details). Users are not required to update the R packages to versions that are more recent than those listed for the RSPARROW compatibility versions in the DESCRIPTION file. However, when a more recent version of R is installed (>R-4.3.1), the latest versions of the library dependencies will be automatically installed with the initial execution of the control script. Note that RSPARROW compatibility with updated R package versions (more recent than those listed in the DESCRIPTION file) is not guaranteed.
...
```{r, eval=TRUE}
path_master <- "C:/DATA/rsparrow_2.0.0_08-22-2023_dyn3/RSPARROW_master"
path_main <- path_master
write.table(file="C:/DATA/rsparrow_2.0.0_08-22-2023_dyn3/RSPARROW_master/myWD.txt",path_main) 
...
* *Adjustment of the model coefficient standard errors of dynamic models for serial correlation.* Serial correlation can exist in the time series of the residuals of dynamic SPARROW models, which can cause somewhat less accurate estimates of the model coefficient standard errors and their statistical significance (e.g., Ator et al., 2019; Schmadel et al., 2021). Ancillary SAS SPARROW code is currently available to adjust the standard errors of the coefficients of the dynamic models for serial correlation (G. Schwarz, USGS, written comm., 2022). A future version of RSPARROW will incorporate these methods for use with RSPARROW dynamic models. 
...
Gorman Sanisaca, Lillian and Alexander, Richard B.  (2023). [RSPARROW 2.0: An R system for SPARROW modeling](https://code.usgs.gov/water/stats/RSPARROW) [Software release]. U.S. Geological Survey. DOI: XXX
